...
+ The average number of total steps taken is **`r d_mean`**.  
+ The median number of total steps taken is **`r d_median`**.  
...
The time series above shows the daily activy pattern of the measure data.  
We can see that most activity is between the **500** and **2000** intervals.  
The interval **`r maxInterval`** contains the maximum average number of steps
taken.
...
After filling this missing values with the respective interval average,
...
# Firstly, we average the 5-minute interval across all days
interval_filled_stats <- group_by(d_filled, interval) %>%
                         summarise(total=sum(steps, na.rm = FALSE),
                                   mean=mean(steps, na.rm = FALSE)
                         )

# Then we plot
with(interval_filled_stats,
     plot(interval, mean, type = "l",
...
The time series above shows the daily activy pattern of the measure data
after the missing data was filled.  
We don't see big changes from the previous chart, since most of the
activity is still between the **500** and **2000** intervals.  

There are changes though:

+ The average number of total steps taken now is **`r d_filled_mean`**.  
+ The median number of total steps taken now is **`r d_filled_median`**.  

## Are there differences in activity patterns between weekdays and weekends?

```{r weekday,echo=TRUE,fig.height=8}

# Create a new factor variable in the dataset with two levels
# “weekday” and “weekend”

# Function that verifies whether a day is weekday (0) or weekend
...
interval_filled_stats2 <- group_by(d_filled, wd, interval) %>%
                          summarise(total=sum(steps, na.rm = FALSE),
                                    mean=mean(steps, na.rm = FALSE)
                         )
